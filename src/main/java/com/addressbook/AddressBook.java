...
/*
 * AddressBook class containing all Use Case operations
 */
public class AddressBook {
	...
	// Helper method to add to maps
	private void addToCityAndStateMap(Contact contact) {
	...
	// Display grouped by city
	public void displayContactsByCity() {
		for (String city : cityMap.keySet()) {
			System.out.println("\nCity: " + city);
			for (Contact c : cityMap.get(city)) {
				System.out.println(c);
			}
		}
	}
	...
	public void displayContactsByState() {
		for (String state : stateMap.keySet()) {
			System.out.println("\nState: " + state);
			for (Contact c : stateMap.get(state)) {
				System.out.println(c);
			}
		}
	}
	...
	public void countContactsByCityInDB(String city) {
		AddressBookDBService dbService = new AddressBookDBService();
		int count = dbService.getCountOfContactsByCity(city);
		System.out.println("Total contacts in city " + city + " in DB: " + count);
	}

	// UC19: Count Contacts by State in DB
	public void countContactsByStateInDB(String state) {
		AddressBookDBService dbService = new AddressBookDBService();
